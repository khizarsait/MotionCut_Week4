...
const markAllCompleteBtn = document.querySelector("button#mark-all-complete");
...
function showTasksList() {
  ...
  if (list.length === 0) {
     ...
     document.querySelectorAll(`li .checkbox input`).forEach(item => {
       item.addEventListener("change", e => {
         const taskId = +e.target.parentElement.parentElement.querySelector('i.edit').dataset.id;
         completeTask(taskId);
       });
     });
   
     document.querySelectorAll(`li .complete-checkbox input`).forEach(item => {
       item.addEventListener("change", e => {
         const eventId = +e.target.parentElement.parentElement.dataset.id;
  ...
  list.reverse().forEach(task => {
     const element = `
       <li class="ui segment grid equal width">
         ...
         <div class="column">
           <i data-id="${task.id}" class="edit outline icon" onclick="showEditModal(${task.id})"></i>
           <i data-id="${task.id}" class="trash alternate outline remove icon" onclick="showRemoveModal(${task.id})"></i>
         </div>
       </li>
     `;
  ...
  document.querySelectorAll(`li .checkbox input`).forEach(item => {
     item.addEventListener("change", e => {
       completeTask(+e.target.parentElement.parentElement.querySelector('i.edit').dataset.id);
     });
  });
...
function completeEvent(id) {
  const eventIndex = list.findIndex(t => t.id === id);
...
function clearCompleteTasks() {
  if (list.length > 0) {
    if (confirm("Are you sure?")) {
      const filteredTasks = list.filter(t => !t.completed);
